...
type Scenario = { id: string; icon: string; name: string; description: string; accent: string; routes: Record<RouteRole, ModelRoute>; effort: { claude: ClaudeEffort; codex: CodexEffort }; recommended?: boolean };
type ToolSettings = { bypassPermissions: boolean; effortLevel: ClaudeEffort };
...
const scenarios: Scenario[] = [
  { id: "daily_coding", icon: "☀", name: "Daily Coding", description: "Balanced direct models for everyday work", accent: "mint", routes: routes(models.opus48, models.sonnet5, models.haiku45), effort: { claude: "medium", codex: "medium" }, recommended: true },
  { id: "heavy_reasoning", icon: "✦", name: "Heavy Reasoning", description: "Maximum planning depth for difficult investigations", accent: "purple", routes: routes(models.opus48, models.sonnet5, models.haiku45), effort: { claude: "max", codex: "xhigh" } },
  { id: "never_stop", icon: "∞", name: "Direct Coding", description: "Long-running implementation with robust reasoning", accent: "blue", routes: routes(models.sol, models.terra, models.luna), effort: { claude: "high", codex: "high" } },
  { id: "premium", icon: "◇", name: "Premium", description: "Highest-quality direct model mapping", accent: "amber", routes: routes(models.opus48, models.sonnet5, models.haiku45), effort: { claude: "high", codex: "high" } },
  { id: "claude_only", icon: "✺", name: "Claude Only", description: "1M-context Claude mapping; no Haiku route", accent: "purple", routes: routes(models.opus48, models.sonnet5, models.sonnet46), effort: { claude: "high", codex: "high" } },
  { id: "codex_only", icon: "◉", name: "Codex Only", description: "Direct Codex tier mapping", accent: "blue", routes: routes(models.sol, models.terra, models.luna), effort: { claude: "high", codex: "high" } },
];
...
function App() {
  ...
  const [toolSettings, setToolSettings] = useState<Record<string, ToolSettings>>({
    claude: { bypassPermissions: false, effortLevel: "medium" },
    codex: { bypassPermissions: false, effortLevel: "medium" },
  ...
  const foundTools = tools.filter(t => t.found);
  const selected = tools.filter(t => t.selected && t.found);
  const activeTool = selected.find(t => t.id === applyTarget) ?? selected[0];
  const activeSettings = toolSettings[activeTool?.id ?? "claude"] ?? { bypassPermissions: false, effortLevel: "medium" };
  ...
  const codexContextWindow = models.terra.contextWindow;
  const defaultModelContextWindow = selectedScenario.routes.sonnet.model.contextWindow;
  const effortOptions: ClaudeEffort[] = activeTool?.id === "codex" ? ["low", "medium", "high", "xhigh"] : ["low", "medium", "high", "xhigh", "max"];
  const requiredModels = useMemo(() => {
    const ids = new Set<string>();
    if (selected.some(tool => tool.id === "claude")) routeRoles.forEach(role => ids.add(selectedScenario.routes[role].model.id));
    if (selected.some(tool => tool.id === "codex")) ids.add(codexModel);
    if (selected.some(tool => ["opencode", "openclaw", "factory", "pi"].includes(tool.id))) ids.add(selectedScenario.routes.sonnet.model.id);
    return [...ids];
  }, [selected, selectedScenario, codexModel]);
  ...
  const codexConfig = useMemo(() => `model = "${codexModel}"\nmodel_provider = "9router"\nmodel_reasoning_effort = "${toolSettings.codex.effortLevel === "max" ? "xhigh" : toolSettings.codex.effortLevel}"\nmodel_context_window = ${codexContextWindow}\nmodel_auto_compact_token_limit = ${Math.floor(codexContextWindow * 0.8)}\napproval_policy = "${toolSettings.codex.bypassPermissions ? "never" : "on-request"}"\nsandbox_mode = "${toolSettings.codex.bypassPermissions ? "danger-full-access" : "workspace-write"}"\n\n[model_providers.9router]\nname = "9router"\nbase_url = "${baseUrl}"\nwire_api = "responses"\nexperimental_bearer_token = "${token ? "••••••••••••" : "<your-9router-token>"}"`, [codexModel, toolSettings.codex, baseUrl, token, codexContextWindow]);
  ...
  const continueSetup = useMemo(() => `name: 9router\nversion: 0.0.1\nschema: v1\nmodels:\n  - name: 9router ${selectedScenario.name}\n    provider: openai\n    model: ${selectedScenario.routes.sonnet.model.id}\n    apiBase: ${baseUrl}\n    apiKey: ${token || "<your-9router-token>"}\n    contextLength: ${defaultModelContextWindow ?? "<verify-context-window>"}\n    capabilities:\n      - tool_use\n    roles:\n      - chat\n      - edit\n      - apply`, [baseUrl, token, selectedScenario, defaultModelContextWindow]);
  const guidedSetup = useMemo(() => {
    const model = selectedScenario.routes.sonnet.model.id;
    if (activeTool?.id === "cline") return <>In Cline Settings, choose <code>OpenAI Compatible</code>, then enter the 9router base URL, API key, and <code>{model}</code>.</>;
    if (activeTool?.id === "roo") return <>In Roo Code provider settings, choose its OpenAI-compatible route and enter the 9router base URL, API key, and <code>{model}</code>.</>;
    if (activeTool?.id === "kilo") return <>In Kilo Code, add an <code>OpenAI Compatible</code> provider with the 9router base URL, API key, and <code>{model}</code>.</>;
    if (activeTool?.id === "continue") return <>Continue accepts an OpenAI-compatible model in its <code>config.yaml</code>. Use the shown base URL, API key, model and context length in Continue’s configuration UI/file.</>;
    if (activeTool?.id === "copilot-vscode") return <>In GitHub Copilot Chat settings, open Model providers, add an OpenAI-compatible provider, then enter the 9router base URL, API key, and <code>{model}</code>.</>;
    if (activeTool?.id === "copilot-cli") return <>Copilot CLI supports OpenAI-compatible BYOK through session environment variables. The app will not silently put a secret into your shell startup file.</>;
    if (activeTool?.id === "cursor") return <>Cursor’s OpenAI base-URL override is global and can disable its built-in OpenAI models, so confirm it in Cursor Settings instead of applying it here.</>;
    if (activeTool?.id === "vibe") return <>Vibe needs both a generic-provider preset in <code>~/.vibe/config.toml</code> and a token environment source, so it remains a guided two-file setup.</>;
    if (activeTool?.id === "hermes") return <>Hermes supports any OpenAI-compatible custom endpoint. Run <code>hermes model</code>, choose Custom endpoint, then enter the 9router URL, API key, <code>{model}</code>, and the displayed context length.</>;
    return <>This tool can use a custom provider, but its config store has no verified safe merge adapter yet, so it will not be changed.</>;
  }, [activeTool?.id, selectedScenario]);
  ...
  async function apply() {
    ...
    try {
      ...
      const result = await invoke<string[]>("apply_configuration", { request: { toolIds: selected.map(t => t.id), routes: Object.fromEntries(routeRoles.map(role => [role, selectedScenario.routes[role].model.id])), token, baseUrl, compactWindow, defaultModelContextWindow, codexContextWindow, toolSettings } });
  ...
  function updateActiveSettings(update: Partial<ToolSettings>) {
    ...
    setToolSettings(current => ({ ...current, [activeTool.id]: { ...(current[activeTool.id] ?? { bypassPermissions: false, effortLevel: "medium" }), ...update } }));
  }
  function chooseScenario(id: string) {
    ...
    setToolSettings(current => ({
      ...current,
      claude: { ...(current.claude ?? { bypassPermissions: false, effortLevel: "medium" }), effortLevel: scenario.effort.claude },
      codex: { ...(current.codex ?? { bypassPermissions: false, effortLevel: "medium" }), effortLevel: scenario.effort.codex },
    }));
  }
